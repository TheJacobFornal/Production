...
export default function Sidebar() {
  ...
  const matchKooperacja  = useMatch('/kooperacja')
  const matchSettings    = useMatch('/ustawienia')
  ...
  const isKooperacja     = !!matchKooperacja
  const isSettings       = !!matchSettings
  ...
  const items = [
    ...
    {
      id:       'kooperacja',
      label:    'Panel Kooperacji',
      icon:     <IconKooperacja />,
      active:   isKooperacja,
      disabled: false,
      onClick:  () => navigate('/kooperacja'),
    },
  ]
    ...
    <>
      ...
      <div style={{
        ...
        <div
          ...
          onMouseEnter={e => {
            const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
            setTip({ label: settingsItem.label, y: rect.top + rect.height / 2 })
          }}
          onMouseLeave={() => setTip(null)}
        >
          {settingsItem.active && (
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: settingsItem.active ? 'rgba(255,255,255,0.18)' : 'transparent',
              color: settingsItem.active ? '#ffffff' : '#bfdbfe',
              transition: 'background 0.12s, color 0.12s',
              outline: 'none',
            }}
            onMouseEnter={e => {
